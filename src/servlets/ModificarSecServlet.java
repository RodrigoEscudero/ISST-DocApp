...
public class ModificarSecServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(paciente != null) {req.getSession().setAttribute("paciente", paciente);
		resp.sendRedirect(req.getContextPath() + "/ModificarPacienteSec.jsp");}
		else if(medico != null) {req.getSession().setAttribute("medico", medico);
		resp.sendRedirect(req.getContextPath() + "/ModificarMedicoSec.jsp");}
		else { resp.sendRedirect(req.getContextPath() + "/LoginSecretaria.jsp");
		}
